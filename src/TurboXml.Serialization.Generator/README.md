...
`TurboXmlFieldBackedPropertyAttribute` optionally configures a context to discover source-visible fields marked by another generator's attribute. Give it the marker's metadata name and optionally its property-name and XML-attribute-string named arguments; the generated reader assigns the public property emitted by that generator without reflection. XML serialization attributes placed on the backing field are honored. A configured string or string collection ignores non-XML entries and may supply restricted `XmlElement("name")` overrides; malformed or unsupported XML-shaped entries report a diagnostic.
