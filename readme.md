...
- By default, this parser rejects DTD and XML directives (`<!DOCTYPE ...>`). Set `XmlParserOptions.IgnoreDtd` to `true` to recognize and skip a document type declaration without loading external resources, processing DTD declarations, or expanding custom entities.
- Generic processing instructions such as `<?xpacket ...?>` or `<?xml-stylesheet ...?>` trigger `IXmlReadHandler.OnProcessingInstruction`. Its default implementation is empty, so existing handlers continue to ignore them. Only the XML declaration `<?xml ...?>` triggers `IXmlReadHandler.OnXmlDeclaration`.
